// Pins the exact golangci-lint a developer and CI both build from, so the two
// cannot drift. The reusable go-check.yml workflow detects this file and runs
// `go -C tools build`; without it CI falls back to a plain string input that
// nothing keeps current.
module github.com/duynhlab/obs-as-code/tools
